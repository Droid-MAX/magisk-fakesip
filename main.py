...
#
# MagiskFakesip build process
#
# 1. Checks if project has a tag that matches fakesip tag
#    Yes -> continue
#    No  -> must tag
...
def main():
    last_fakesip_tag = util.get_last_fakesip_tag()
    last_project_tag = util.get_last_project_tag()
    new_project_tag = "0"
    
    force_release = os.getenv('FORCE_RELEASE', 'false').lower() == 'true'
    needs_update = last_fakesip_tag != util.strip_revision(last_project_tag)
    
    if needs_update or force_release:
        new_project_tag = util.get_next_revision(last_fakesip_tag)
        print(f"Update needed to {new_project_tag}")
        
        if needs_update:
            print(f"Reason: Fakesip updated from {util.strip_revision(last_project_tag)} to {last_fakesip_tag}")
        else:
    ...
    build.do_build(last_fakesip_tag, new_project_tag)
