...
on_install() {
  ...
  busybox unzip -qq -o "$ZIPFILE" "files/fakesip-$F_ARCH" -j -d "$BIN_DIR"
  busybox unzip -qq -o "$ZIPFILE" "files/fakesip.conf" -j -d "$CONF_DIR"
  mv "$BIN_DIR/fakesip-$F_ARCH" "$BIN_DIR/fakesip"
}
...
set_permissions() {
  ...
  # Custom permissions
  set_perm $BIN_DIR/fakesip 0 2000 0755 u:object_r:system_file:s0
  set_perm $CONF_DIR/fakesip.conf 0 2000 0644 u:object_r:system_file:s0
}
